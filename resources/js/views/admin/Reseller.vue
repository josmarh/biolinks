...
const contents = computed(() => resellerStore.state.users)
...
watch(emailSearch, (newVal, oldVal) => {
    setTimeout(() => {
        resellerStore.dispatch('getUsers', {url: newVal})
    }, 1500)
});
...
function saveUser(item, item2) {
    resellerStore
        ...
        .then((res) => {
            ...
            resellerStore.dispatch('getUsers')
        })
        .catch((err) => {
            if(err.response) {
                if (err.response.data) {
...
const getForPage = (ev,link) => {
    ...
    resellerStore.dispatch('getUsers', {url: link.url});
}
