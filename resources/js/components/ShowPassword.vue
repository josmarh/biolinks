<template>
    <div class="flex absolute inset-y-0 right-0 items-center pr-3 cursor-pointer">
        <svg v-if="type == 'password'"
            xmlns="http://www.w3.org/2000/svg" @click="showPassword(elem)"
            class="h-5 w-6 text-gray-600 dark:text-gray-400 mt-6" 
            fill="none" 
            viewBox="0 0 24 24" 
            stroke="currentColor" 
            stroke-width="2">
        ...
        <svg v-else xmlns="http://www.w3.org/2000/svg" @click="showPassword(elem)"
            class="h-5 w-6 text-gray-600 dark:text-gray-400 mt-6" 
            fill="none" 
            viewBox="0 0 24 24" 
            stroke="currentColor" 
...
const showPassword = (elm) => {
    ...
    if(passElm.value !== ''){
        if (passElm.getAttribute('type') == 'password') {
            ...
            type.value = passElm.getAttribute('type')
        } else {
