<template>
    <modal-layout title="Add a Twitch video" :show-form="open" @close-modal="closeModal">
        <form @submit.prevent="newTwitch">
            <p class="text-md text-gray-600 font-normal mt-8 tracking-tight">
                Paste in your Twitch URL and we will show it as a video on your profile.
            </p>
            <link-field 
                :data="model.sectionFields.twitchUrl" 
                :required="linkRequired"
                label="Twitch URL"
                placeholder="https://www.twitch.tv/dota2ti"
                @update="updateLink" 
            />
...
function updateLink(data) {
    model.value.sectionFields.twitchUrl = data
}
