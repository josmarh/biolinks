    ...
    <div>
        <div v-for="(item, i) in model" :key="i"
            ...
            <div class="w-full mt-2">
                <select :id="'device-select-' + i" v-model="model[i].device"
            ...
            <div class="w-full mt-2">
                <input type="url" :id="`device-url-${i}`" v-model="model[i].url"
...
const emit = defineEmits(['addDevice', 'updateDevice']);
const props = defineProps({
    addedDevices: Array,
});

let model = ref(props.addedDevices)

watch(() => props.addedDevices, (newVal, oldVal) => {
    model.value = newVal
}, {deep:true})
