    ...
    <div>
        <div v-for="(item, i) in model" :key="i"
            ...
            <div class="w-full mt-2">
                <select :id="'blang-select-' + i" v-model="model[i].lang"
                    ...
                    <option v-for="item in languages" :key="item.id" 
            ...
            <div class="w-full mt-2">
                <input type="url" :id="`blang-url-${i}`" v-model="model[i].url"
        ...
        <button type="button" @click="addBlang"
...
const emit = defineEmits(['addBlang', 'updateBlang']);
const props = defineProps({
    addedBlangs: Array,
    languages: Array
});

let model = ref(props.addedBlangs);

watch(() => props.addedBlangs, (newVal, oldVal) => {
    model.value = newVal
}, {deep:true})
